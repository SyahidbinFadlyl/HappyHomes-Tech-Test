<script>
export default {
  props: {
    product: Object,
  },
};
</script>

<template>
  <div class="containerProduct">
    <div class="imageContainer">
      <img :src="product.thumbnail" class="img" alt="image" />
    </div>
    <div>
      <div class="detail">
        <p class="producTitle">{{ product.title }}</p>
      </div>
      <div class="productDescription">
        <p class="description">{{ product.description }}</p>
      </div>
      <div class="productDescription">
        <p class="tagPrice">
          Price : <span class="productPrice">${{ product.price }}</span>
        </p>
      </div>
      <div class="AddToChart">
        <svg class="icon">
          <path
            d="M11,9H13V6H16V4H13V1H11V4H8V6H11M7,18A2,2 0 0,0 5,20A2,2 0 0,0 7,22A2,2 0 0,0 9,20A2,2 0 0,0 7,18M17,18A2,2 0 0,0 15,20A2,2 0 0,0 17,22A2,2 0 0,0 19,20A2,2 0 0,0 17,18M7.17,14.75L7.2,14.63L8.1,13H15.55C16.3,13 16.96,12.59 17.3,11.97L21.16,4.96L19.42,4H19.41L18.31,6L15.55,11H8.53L8.4,10.73L6.16,6L5.21,4L4.27,2H1V4H3L6.6,11.59L5.25,14.04C5.09,14.32 5,14.65 5,15A2,2 0 0,0 7,17H19V15H7.42C7.29,15 7.17,14.89 7.17,14.75Z"
          ></path>
        </svg>
        <p class="addToChart">ADD TO CHART</p>
        <svg class="shareIcon">
          <path
            d="M18 16.08C17.24 16.08 16.56 16.38 16.04 16.85L8.91 12.7C8.96 12.47 9 12.24 9 12S8.96 11.53 8.91 11.3L15.96 7.19C16.5 7.69 17.21 8 18 8C19.66 8 21 6.66 21 5S19.66 2 18 2 15 3.34 15 5C15 5.24 15.04 5.47 15.09 5.7L8.04 9.81C7.5 9.31 6.79 9 6 9C4.34 9 3 10.34 3 12S4.34 15 6 15C6.79 15 7.5 14.69 8.04 14.19L15.16 18.34C15.11 18.55 15.08 18.77 15.08 19C15.08 20.61 16.39 21.91 18 21.91S20.92 20.61 20.92 19C20.92 17.39 19.61 16.08 18 16.08M18 4C18.55 4 19 4.45 19 5S18.55 6 18 6 17 5.55 17 5 17.45 4 18 4M6 13C5.45 13 5 12.55 5 12S5.45 11 6 11 7 11.45 7 12 6.55 13 6 13M18 20C17.45 20 17 19.55 17 19S17.45 18 18 18 19 18.45 19 19 18.55 20 18 20Z"
          ></path>
        </svg>
      </div>
    </div>
  </div>
</template>

<style scoped>
/* .img {
  height: 300px;
  width: 250px;
  max-height: 300px;
  max-width: 250px;
}
.icon {
  width: 21px;
  height: 21px;
} */
</style>
